        ...
        if ("\\" !== E)
          if ("{" !== E)
            if ("}" !== E)
              if (":" !== E)
                if ("(" !== E)
                  t.push({ type: "CHAR", index: F, value: u[F++] });
                else {
                  var r = 1,
                    i = "",
                    s = F + 1,
        ...
        for (
          ...
          if (
            ...
            if (this.state === f.INIT) {
              this.rewind(),
                this.isHashPrefix()
                  ? this.changeState(f.HASH, 1)
                  : this.isSearchPrefix()
                  ? (this.changeState(f.SEARCH, 1),
                    (this.internalResult.hash = ""))
                  : (this.changeState(f.PATHNAME, 0),
            ...
            switch (this.state) {
              case f.INIT:
                this.isProtocolSuffix() &&
                  ...
                  (this.internalResult.search = ""),
                  (this.internalResult.hash = ""),
                  this.rewindAndSetState(f.PROTOCOL));
                break;
              case f.PROTOCOL:
                if (this.isProtocolSuffix()) {
                  ...
                  this.shouldTreatAsStandardURL &&
                    ...
                    this.nextIsAuthoritySlashes()
                      ? ((u = f.AUTHORITY), (D = 3))
                      : this.shouldTreatAsStandardURL && (u = f.AUTHORITY),
                    this.changeState(u, D);
                }
                break;
              case f.AUTHORITY:
                this.isIdentityTerminator()
                  ? this.rewindAndSetState(f.USERNAME)
              ...
              case f.USERNAME:
                this.isPasswordPrefix()
                  ? this.changeState(f.PASSWORD, 1)
                  : this.isIdentityTerminator() &&
                    this.changeState(f.HOSTNAME, 1);
                break;
              case f.PASSWORD:
        ...
        if ((e = C.value) in this.pattern) {
          ...
          switch (((this.keys[e] = []), e)) {
            ...
            case "username":
              ...
              break;
            case "password":
              Object.assign(E, B), (E.encodePart = S);
              break;
            case "hostname":
          ...
          try {
            var i = n(r, E);
            (this.regexp[e] = s(i, this.keys[e], E)),
    ...
    (t.exec = function (u, D) {
      ...
      if (void 0 !== u) {
        ...
        for (e in ((C.inputs = D ? [u, D] : [u]), this.pattern)) {
          ...
          if (!A) return null;
          for (
            var E, n = {}, r = F(this.keys[e].entries());
            !(E = r()).done;
